...
(defmacro parse-pattern-ce (name pattern-ce &optional (binding (gensym)))
  (declare (ignore name))
  `(progn
     ;; Update bindings
     (push (list ,binding (length fact-bindings)) fact-bindings)
     (dolist (slot (cdr ,pattern-ce))
       (let ((value (cadr slot))
	     (slot-name (car slot))
	     (template (car ,pattern-ce)))
	 (when (variable-p value)
	   (push (list value template slot-name ,binding) variable-bindings))))))
